...
@Suite struct WKTBoolValueTests {
  struct WrappedBoolValueEncode: Encodable {
    let value: GoogleWKT.WKTBoolValue?
  }
  ...
  func boolValueAnyUnpack() throws {
    ...
    let decoder = _ProtoJSONDecoder()
    let wrapped = try decoder.decode(WKTBoolValueTests.WrappedAny.self, from: data)
    let any = wrapped.content
    #expect(any.typeUrl == "type.googleapis.com/google.protobuf.BoolValue")

    let got = try WKTBoolValue(fromAny: any)
    let want = true
    #expect(got == want)
  }
  ...
  @Test func boolValueAnyUnpackMismatchedUrl() throws {
    ...
    let wrapped = try decoder.decode(WKTBoolValueTests.WrappedAny.self, from: data)
    let any = wrapped.content
    let error = #expect(throws: WKTAnyError.self) { let _ = try WKTBoolValue(fromAny: any) }
    #expect(error == .mismatchedTypeUrl)
  }

  @Test("Pack BoolValue into Any")
  func boolValueAnyPack() throws {
    let input = WKTBoolValue(true)
    let any = try WKTAny(fromMessage: input)
    let wrapped = WKTBoolValueTests.WrappedAny(content: any)
    let encoder = _ProtoJSONEncoder()
