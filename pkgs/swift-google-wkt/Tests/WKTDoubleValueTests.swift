...
@Suite struct WKTDoubleValueTests {
  struct WrappedDoubleValueEncode: Encodable {
    let value: GoogleWKT.WKTDoubleValue?
  }
  ...
  func doubleValueAnyPack() throws {
    let input = WKTDoubleValue(123.45)
    let any = try WKTAny(fromMessage: input)
    let wrapped = WKTDoubleValueTests.WrappedAny(content: any)
    let encoder = _ProtoJSONEncoder()
