...
@Suite struct WKTUInt32ValueTests {
  struct WrappedUInt32ValueEncode: Encodable {
    let value: GoogleWKT.WKTUInt32Value?
  }
  ...
  func uint32ValueAnyUnpackNumber() throws {
    ...
    let decoder = _ProtoJSONDecoder()
    let wrapped = try decoder.decode(WKTUInt32ValueTests.WrappedAny.self, from: data)
    let any = wrapped.content
    #expect(any.typeUrl == "type.googleapis.com/google.protobuf.UInt32Value")

    let got = try WKTUInt32Value(fromAny: any)
    let want = UInt32(123)
    #expect(got == want)
  }
  ...
  @Test func uint32ValueAnyUnpackMismatchedUrl() throws {
    ...
    let wrapped = try decoder.decode(WKTUInt32ValueTests.WrappedAny.self, from: data)
    let any = wrapped.content
    let error = #expect(throws: WKTAnyError.self) { let _ = try WKTUInt32Value(fromAny: any) }
    #expect(error == .mismatchedTypeUrl)
  }

  @Test("Pack UInt32Value into Any")
  func uint32ValueAnyPack() throws {
    let input = WKTUInt32Value(123)
    let any = try WKTAny(fromMessage: input)
    let wrapped = WKTUInt32ValueTests.WrappedAny(content: any)
    let encoder = _ProtoJSONEncoder()
  ...
  func uint32ValueBoundaries(_ value: UInt32) throws {
    ...
    let decoder = _ProtoJSONDecoder()
    let decodedWrapped = try decoder.decode(WKTUInt32ValueTests.WrappedAny.self, from: data)
