...
@Suite struct WKTFloatValueTests {
  struct WrappedFloatValueEncode: Encodable {
    let value: GoogleWKT.WKTFloatValue?
  }
  ...
  @Test(
    ...
    arguments: [
      (
        WKTFloatValue(123.45),
        #"{"content":{"@type":"type.googleapis.com/google.protobuf.FloatValue","value":123.45}}"#
      ),
      (
        WKTFloatValue(123456.75),
        #"{"content":{"@type":"type.googleapis.com/google.protobuf.FloatValue","value":123456.75}}"#
      ),
    ]
  )
  func floatValueAnyPack(input: WKTFloatValue, want: String) throws {
    let any = try WKTAny(fromMessage: input)
    let wrapped = WKTFloatValueTests.WrappedAny(content: any)
    let encoder = _ProtoJSONEncoder()
