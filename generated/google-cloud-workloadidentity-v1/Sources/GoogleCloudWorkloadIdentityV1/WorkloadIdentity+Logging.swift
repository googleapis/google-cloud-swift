...
extension Clients {
  final class WorkloadIdentityLogging: WorkloadIdentityStub {
    let inner: any WorkloadIdentityStub
    let logger: Logger

    public init(_ inner: any WorkloadIdentityStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-workloadidentity-v1"
      logger[metadataKey: "gcp.client.service"] = "workloadidentity"
      logger[metadataKey: "gcp.experimental.swift.client"] = "WorkloadIdentity"
      self.inner = inner
      self.logger = logger
    }
    ...
    public func generateServiceAgents(
      ...
      try await self._intercept(
        request: request,
        options: options,
        name: "generateServiceAgents",
        action: {
          (r: GenerateServiceAgentsRequest, o: GoogleCloudGax.RequestOptions) async throws
            -> GoogleLongRunning.Operation
          in
          return try await self.inner.generateServiceAgents(request: r, options: o)
        })
    }
