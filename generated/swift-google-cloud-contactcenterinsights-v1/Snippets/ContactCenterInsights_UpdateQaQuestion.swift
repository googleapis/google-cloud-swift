...
func sample(
  ...
  revisionId: String, qaQuestionId: String
) async throws {
  let response = try await client.updateQaQuestion(
    request: UpdateQaQuestionRequest()
      .with {
        $0.qaQuestion = QaQuestion().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/qaScorecards/\(qaScorecardId)/revisions/\(revisionId)/qaQuestions/\(qaQuestionId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        qaScorecardId: "[placeholder]", revisionId: "[placeholder]", qaQuestionId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
