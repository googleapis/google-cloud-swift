...
extension Clients {
  final class WidgetServiceTransport: WidgetServiceStub {
    ...
    public func generateChatToken(
      request: GenerateChatTokenRequest, options: GoogleGax.RequestOptions
      ...
      let (path, query, configure, omitted) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0):generateChatToken"
          let query = [
      ...
      return try await req.rpc(
        GoogleCloudCESV1.GenerateChatTokenResponse.self, timeout: options.attemptTimeout
      ).get()
    }
