...
public struct QueryUserRootContentsResponse: Codable, Equatable, GoogleWKT._AnyPackable,
  Sendable
{
  /// List of entries in the folder.
  public var entries: [QueryUserRootContentsResponse.RootContentsEntry] = []
  ...
  ///
  /// ```
  /// let value = QueryUserRootContentsResponse().with { $0.entries = ... }
  /// ```
  ...
  public struct RootContentsEntry: Codable, Equatable, GoogleWKT._AnyPackable,
    Sendable
  {
    /// The content entry.
    ...
    ///
    /// ```
    /// let value = RootContentsEntry().with { $0.folder = ... }
    /// ```
    public func with(_ config: (inout Self) throws -> Swift.Void) rethrows -> Self {
      var copy = self
      try config(&copy)
...
extension QueryUserRootContentsResponse: GoogleGax._PaginatedResponse {
