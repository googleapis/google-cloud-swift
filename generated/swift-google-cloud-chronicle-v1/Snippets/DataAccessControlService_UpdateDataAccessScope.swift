...
func sample(
  ...
  let response = try await client.updateDataAccessScope(
    request: UpdateDataAccessScopeRequest()
      .with {
        $0.dataAccessScope = DataAccessScope().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/instances/\(instanceId)/dataAccessScopes/\(dataAccessScopeId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        instanceId: "[placeholder]", dataAccessScopeId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
