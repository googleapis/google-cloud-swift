...
func sample(
  ...
  let response = try await client.updateDataAccessLabel(
    request: UpdateDataAccessLabelRequest()
      .with {
        $0.dataAccessLabel = DataAccessLabel().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/instances/\(instanceId)/dataAccessLabels/\(dataAccessLabelId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        instanceId: "[placeholder]", dataAccessLabelId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
