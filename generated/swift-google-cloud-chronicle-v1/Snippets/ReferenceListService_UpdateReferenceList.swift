...
func sample(
  ...
  let response = try await client.updateReferenceList(
    request: UpdateReferenceListRequest()
      .with {
        $0.referenceList = ReferenceList().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/instances/\(instanceId)/referenceLists/\(referenceListId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        instanceId: "[placeholder]", referenceListId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
