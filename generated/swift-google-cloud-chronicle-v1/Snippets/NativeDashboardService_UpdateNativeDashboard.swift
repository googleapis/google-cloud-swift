...
func sample(
  ...
  let response = try await client.updateNativeDashboard(
    request: UpdateNativeDashboardRequest()
      .with {
        $0.nativeDashboard = NativeDashboard().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/instances/\(instanceId)/nativeDashboards/\(dashboardId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        instanceId: "[placeholder]", dashboardId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
