// Code generated by sidekick. DO NOT EDIT.
//
// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

import Foundation
@_spi(GoogleCloudInternal) public import GoogleWKT

/// TrellixStarXAuthentication contains a oneof with all of the authentication
/// types supported by Trellix *X devices.
public struct TrellixStarXAuthentication: Codable, Equatable, GoogleWKT._AnyPackable,
  Sendable
{
  /// One of multiple potential auth types.
  public var authType: OneOf_AuthType? = nil

  @_spi(GoogleCloudInternal) public var _unknownFields: GoogleWKT._UnknownFields = .init()

  /// Initialize a new instance of `TrellixStarXAuthentication`.
  public init() {}

  /// Use `config` to return a new instance of this object, with some fields updated.
  ///
  /// Commonly used to initialize the value, for example:
  ///
  /// ```
  /// let value = TrellixStarXAuthentication().with { $0.msso = ... }
  /// ```
  public func with(_ config: (inout Self) throws -> Swift.Void) rethrows -> Self {
    var copy = self
    try config(&copy)
    return copy
  }

  private struct CodingKeys: CodingKey {
    var stringValue: Swift.String
    var intValue: Swift.Int? { nil }
    init(stringValue: Swift.String) { self.stringValue = stringValue }
    init?(intValue: Swift.Int) { nil }

    static let msso = CodingKeys(stringValue: "msso")
    static let trellixIam = CodingKeys(stringValue: "trellixIam")
    static let trellixLocal = CodingKeys(stringValue: "trellixLocal")

    static let _knownKeys: Set<Swift.String> = [
      "msso",
      "trellixIam",
      "trellixLocal",
    ]
  }

  public init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: CodingKeys.self)

    var authType: OneOf_AuthType? = nil
    let authTypeCheckAndSet = {
      if authType != nil {
        throw DecodingError.dataCorrupted(
          DecodingError.Context(
            codingPath: decoder.codingPath,
            debugDescription: "Multiple values set for oneof 'authType'"))
      }
      authType = $0
    }
    if let msso = try container.decodeIfPresent(MssoAuthentication?.self, forKey: .msso) {
      try authTypeCheckAndSet(.msso(msso))
    }
    if let trellixIam = try container.decodeIfPresent(
      TrellixIAMAuthentication?.self, forKey: .trellixIam)
    {
      try authTypeCheckAndSet(.trellixIam(trellixIam))
    }
    if let trellixLocal = try container.decodeIfPresent(
      TrellixLocalAuthentication?.self, forKey: .trellixLocal)
    {
      try authTypeCheckAndSet(.trellixLocal(trellixLocal))
    }
    self.authType = authType
    for key in container.allKeys where !CodingKeys._knownKeys.contains(key.stringValue) {
      self._unknownFields.json[key.stringValue] = try container.decode(
        GoogleWKT.WKTValue.self, forKey: key)
    }
  }

  public func encode(to encoder: Encoder) throws {
    var container = encoder.container(keyedBy: CodingKeys.self)

    if let choice = self.authType {
      switch choice {
      case .msso(let value):
        try container.encode(value, forKey: .msso)
      case .trellixIam(let value):
        try container.encode(value, forKey: .trellixIam)
      case .trellixLocal(let value):
        try container.encode(value, forKey: .trellixLocal)
      }
    }
    for (key, value) in self._unknownFields.json {
      try container.encode(value, forKey: CodingKeys(stringValue: key))
    }
  }

  /// One of multiple potential auth types.
  public enum OneOf_AuthType: Codable, Equatable, Sendable {
    /// Input only. MssoAuthentication auth type.
    indirect case msso(MssoAuthentication?)
    /// Input only. TrellixIAMAuthentication auth type.
    indirect case trellixIam(TrellixIAMAuthentication?)
    /// Input only. TrellixLocalAuthentication auth type.
    indirect case trellixLocal(TrellixLocalAuthentication?)
  }

  public static var _anyTypeUrl: Swift.String {
    return "type.googleapis.com/google.cloud.chronicle.v1.TrellixStarXAuthentication"
  }
  public init(fromAny any: GoogleWKT.WKTAny) throws {
    self = try GoogleWKT._slowAnyDeserialize(Self.self, from: any)
  }
  public func _pack() throws -> GoogleWKT.WKTStruct {
    return try GoogleWKT._slowAnySerialize(message: self)
  }
}
