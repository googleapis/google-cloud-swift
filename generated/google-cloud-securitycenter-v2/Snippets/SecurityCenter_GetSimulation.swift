...
{
  let response = try await client.getSimulation(
    request: GetSimulationRequest()
      .with {
        $0.name = "organizations/\(organizationId)/simulations/\(simulationId)"
      }
  )
...
struct SnippetRunner {
  static func main() async throws {
    do {
      ...
      try await sample(
        client: client, organizationId: "[placeholder]", simulationId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
