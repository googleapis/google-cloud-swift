...
func sample(
  ...
  let response = try await client.getColumnDataProfile(
    request: GetColumnDataProfileRequest()
      .with {
        $0.name =
          "organizations/\(organizationId)/locations/\(locationId)/columnDataProfiles/\(columnDataProfileId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        columnDataProfileId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
