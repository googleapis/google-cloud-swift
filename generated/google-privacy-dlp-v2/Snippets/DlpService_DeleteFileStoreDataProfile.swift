...
func sample(
  client: DlpServiceClient, organizationId: String, locationId: String,
  fileStoreDataProfileId: String
) async throws {
  try await client.deleteFileStoreDataProfile(
    request: DeleteFileStoreDataProfileRequest()
      .with {
        $0.name =
          "organizations/\(organizationId)/locations/\(locationId)/fileStoreDataProfiles/\(fileStoreDataProfileId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        fileStoreDataProfileId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
