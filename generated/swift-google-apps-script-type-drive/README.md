...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-apps-script-type-drive.git --from 0.2.0
```

Then add `GoogleAppsScriptTypeDrive` to your target's dependencies:

```bash
swift package add-target-dependency GoogleAppsScriptTypeDrive <target-name> --package swift-google-apps-script-type-drive
```
