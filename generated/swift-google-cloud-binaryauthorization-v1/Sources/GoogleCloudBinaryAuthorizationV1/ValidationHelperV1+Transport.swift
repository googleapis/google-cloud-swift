...
extension Clients {
  final class ValidationHelperV1Transport: ValidationHelperV1Stub {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.attestor as Swift.String?,
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/attestors/"), .singleWildcard,
              ],
              fieldName: "attestor")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0):validateAttestationOccurrence"
          let query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          return (path, query)
        }() {
          return (candidate.0, candidate.1, { $0.setMethod(.POST) }, ["attestor"])
        }
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              ...
              matching: [.literal("projects/"), .singleWildcard, .literal("/policy")],
              fieldName: "resource")
          else {
            return nil
          }
