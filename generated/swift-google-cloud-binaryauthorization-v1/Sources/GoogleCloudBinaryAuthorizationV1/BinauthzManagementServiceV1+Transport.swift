...
extension Clients {
  final class BinauthzManagementServiceV1Transport: BinauthzManagementServiceV1Stub {
    ...
    public func createAttestor(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/attestors"
          var query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
    ...
    public func updateAttestor(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.attestor.map({ $0.name }),
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/attestors/"), .singleWildcard,
              ],
              fieldName: "attestor.name")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0)"
