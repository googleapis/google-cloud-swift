...
func sample(
  ...
  entitlementId: String, grantId: String
) async throws {
  let response = try await client.getGrant(
    request: GetGrantRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/entitlements/\(entitlementId)/grants/\(grantId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        entitlementId: "[placeholder]", grantId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
