...
func sample(
  ...
  let poller = try await client.updateLbEdgeExtensionPollingUntilDone(
    request: UpdateLbEdgeExtensionRequest()
      .with {
        $0.lbEdgeExtension = LbEdgeExtension().with {
          $0.name =
...
struct SnippetRunner {
  static func main() async throws {
    do {
      let client = try GoogleCloudNetworkServicesV1.DepServiceClient()
      try await sample(
        client: client, projectId: "[placeholder]", locationId: "[placeholder]",
        lbEdgeExtensionId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
