...
func sample(
  ...
  let response = try await client.getPartnerPermissions(
    request: GetPartnerPermissionsRequest()
      .with {
        $0.name =
          "organizations/\(organizationId)/locations/\(locationId)/customers/\(customerId)/workloads/\(workloadId)/partnerPermissions"
      }
  )
