...
func sample(
  ...
  let response = try await client.getMetadataImport(
    request: GetMetadataImportRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/services/\(serviceId)/metadataImports/\(metadataImportId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        serviceId: "[placeholder]", metadataImportId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
