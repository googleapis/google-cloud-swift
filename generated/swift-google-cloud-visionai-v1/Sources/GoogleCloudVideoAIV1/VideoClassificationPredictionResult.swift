  ...
  Sendable
{
  ...
  ///
  /// ```
  /// let value = VideoClassificationPredictionResult().with { $0.segmentStartTime = ... }
  /// ```
  ...
  public struct IdentifiedClassification: Codable, Equatable, GoogleWKT._AnyPackable,
    Sendable
  {
    ...
    /// Initialize a new instance of `IdentifiedClassification`.
    public init() {}
    ...
    ///
    /// ```
    /// let value = IdentifiedClassification().with { $0.id = ... }
    /// ```
    public func with(_ config: (inout Self) throws -> Swift.Void) rethrows -> Self {
      var copy = self
      try config(&copy)
