...
func sample(
  ...
  let poller = try await client.deleteClientTlsPolicyPollingUntilDone(
    request: DeleteClientTlsPolicyRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/clientTlsPolicies/\(clientTlsPolicyId)"
      }
  )
