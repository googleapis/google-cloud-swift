...
func sample(
  ...
  let poller = try await client.updateSecurityProfile(
    withPolling: UpdateSecurityProfileRequest()
      .with {
...
struct SnippetRunner {
  static func main() async throws {
    do {
      let client = try GoogleCloudNetworkSecurityV1.SecurityProfileGroupServiceClient()
      try await sample(
        client: client, organizationId: "[placeholder]", locationId: "[placeholder]",
        securityProfileId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
