...
func sample(
  ...
  restorePlanBindingId: String
) async throws {
  let response = try await client.getRestorePlanBinding(
    request: GetRestorePlanBindingRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/restoreChannels/\(restoreChannelId)/restorePlanBindings/\(restorePlanBindingId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        restoreChannelId: "[placeholder]", restorePlanBindingId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
