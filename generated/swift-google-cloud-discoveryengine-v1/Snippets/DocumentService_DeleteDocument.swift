...
// limitations under the License.
  ...
  func sample(
    client: DocumentServiceClient, projectId: String, locationId: String, dataStoreId: String,
    branchId: String, documentId: String
  ) async throws {
    try await client.deleteDocument(
      request: DeleteDocumentRequest()
        .with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/dataStores/\(dataStoreId)/branches/\(branchId)/documents/\(documentId)"
        }
    )
