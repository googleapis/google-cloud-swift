...
// limitations under the License.
  ...
  func sample(
    ...
    dataStoreId: String, conversationId: String
  ) async throws {
    let response = try await client.updateConversation(
      request: UpdateConversationRequest()
        .with {
          $0.conversation = Conversation().with {
            $0.name =
              "projects/\(projectId)/locations/\(locationId)/dataStores/\(dataStoreId)/conversations/\(conversationId)"
          }
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          dataStoreId: "[placeholder]", conversationId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }
