...
// limitations under the License.
  ...
  extension Clients {
    final class GroundedGenerationServiceTransport: GroundedGenerationServiceStub {
      ...
      public func streamGenerateGroundedContent(
        ...
        let (path, query, configure, omitted) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/v1/\(pathVariable0):streamGenerateGroundedContent"
            let query = [
        ...
        return try await req.rpc(
          GoogleCloudDiscoveryEngineV1.GenerateGroundedContentResponse.self,
      ...
      public func generateGroundedContent(
        ...
        let (path, query, configure, omitted) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/v1/\(pathVariable0):generateGroundedContent"
            let query = [
              URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
            ]
            return (path, query)
      ...
      public func checkGrounding(
        ...
        let (path, query, configure, omitted) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            guard
              let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
                request.groundingConfig as Swift.String?,
                matching: [
                  .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                  .literal("/groundingConfigs/"), .singleWildcard,
                ],
                fieldName: "grounding_config")
            else {
              return nil
            }
            let path = "/v1/\(pathVariable0):check"
            let query = [
              URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
            ]
            return (path, query)
          }() {
            return (candidate.0, candidate.1, { $0.setMethod(.POST) }, ["groundingConfig"])
          }
          var paths: [GoogleGax.PathMismatch] = []
          do {
            var builder = GoogleGax._PathMismatchBuilder()
            builder.maybeAdd(
              ...
              fieldName: "grounding_config",
              expecting: "projects/*/locations/*/groundingConfigs/*"
            )
            paths.append(builder.build())
          }
          throw GoogleGax.RequestError.binding(GoogleGax.BindingError(paths: paths))
        ...
        try req.setBody(json: request, omitting: omitted)
        return try await req.rpc(
          GoogleCloudDiscoveryEngineV1.CheckGroundingResponse.self, timeout: options.attemptTimeout
        ).get()
      }
