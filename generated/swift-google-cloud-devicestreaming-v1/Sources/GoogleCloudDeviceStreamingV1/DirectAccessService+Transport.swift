...
extension Clients {
  final class DirectAccessServiceTransport: DirectAccessServiceStub {
    ...
    public func createDeviceSession(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/deviceSessions"
          var query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          let encoder = GoogleGax._QueryParameterEncoder()
          query.append(
            contentsOf: try encoder.encode(request.deviceSessionId, prefix: "deviceSessionId"))
          return (path, query)
        }() {
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              ...
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/deviceSessions/"),
                .singleWildcard,
              ],
              fieldName: "name")
          else {
        ...
        do {
          ...
          builder.maybeAdd(
            request.name as Swift.String?,
            matching: [
              .literal("projects/"), .singleWildcard, .literal("/deviceSessions/"), .singleWildcard,
            ],
            fieldName: "name",
            expecting: "projects/*/deviceSessions/*"
          )
          paths.append(builder.build())
        }
        throw GoogleGax.RequestError.binding(GoogleGax.BindingError(paths: paths))
    ...
    public func updateDeviceSession(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.deviceSession.map({ $0.name }),
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/deviceSessions/"),
                .singleWildcard,
              ],
              fieldName: "device_session.name")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0)"
