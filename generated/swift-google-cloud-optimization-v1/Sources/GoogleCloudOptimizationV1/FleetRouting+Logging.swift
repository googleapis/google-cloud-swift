...
extension Clients {
  final class FleetRoutingLogging: FleetRoutingStub {
    let inner: any FleetRoutingStub
    let logger: Logger

    public init(_ inner: any FleetRoutingStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-optimization-v1"
      logger[metadataKey: "gcp.client.service"] = "cloudoptimization"
      logger[metadataKey: "gcp.experimental.swift.client"] = "FleetRouting"
      self.inner = inner
      self.logger = logger
    }
