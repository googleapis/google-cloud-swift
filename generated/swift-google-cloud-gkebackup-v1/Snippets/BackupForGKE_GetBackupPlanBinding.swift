...
func sample(
  ...
  let response = try await client.getBackupPlanBinding(
    request: GetBackupPlanBindingRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/backupChannels/\(backupChannelId)/backupPlanBindings/\(backupPlanBindingId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        backupChannelId: "[placeholder]", backupPlanBindingId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
