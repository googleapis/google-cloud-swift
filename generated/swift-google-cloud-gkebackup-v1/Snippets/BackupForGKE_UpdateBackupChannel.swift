...
func sample(
  ...
  let poller = try await client.updateBackupChannel(
    withPolling: UpdateBackupChannelRequest()
      .with {
        $0.backupChannel = BackupChannel().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/backupChannels/\(backupChannelId)"
        }
