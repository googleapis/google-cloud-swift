// Code generated by sidekick. DO NOT EDIT.
//
// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

import Foundation
@_spi(GoogleCloudInternal) public import GoogleWKT

/// Message describing Database object.
public struct Database: Codable, Equatable, GoogleWKT._AnyPackable,
  Sendable
{
  /// Identifier. Name of the resource in the form of
  /// `projects/{project}/locations/{location}/clusters/{cluster}/databases/{database}`.
  public var name: Swift.String = Swift.String()

  /// Optional. Immutable. Charset for the database.
  /// This field can contain any PostgreSQL supported charset name.
  /// Example values include "UTF8", "SQL_ASCII", etc.
  public var charset: Swift.String = Swift.String()

  /// Optional. Immutable. lc_collate for the database.
  /// String sort order.
  /// Example values include "C", "POSIX", etc.
  public var collation: Swift.String = Swift.String()

  /// Optional. Immutable. lc_ctype for the database.
  /// Character classification (What is a letter? The upper-case equivalent?).
  /// Example values include "C", "POSIX", etc.
  public var characterType: Swift.String = Swift.String()

  /// Input only. Immutable. Template of the database to be used for creating a
  /// new database.
  public var databaseTemplate: Swift.String = Swift.String()

  /// Optional. Whether the database is a template database.
  public var isTemplateDatabase: Swift.Bool? = nil

  @_spi(GoogleCloudInternal) public var _unknownFields: GoogleWKT._UnknownFields = .init()

  /// Initialize a new instance of `Database`.
  public init() {}

  /// Use `config` to return a new instance of this object, with some fields updated.
  ///
  /// Commonly used to initialize the value, for example:
  ///
  /// ```
  /// let value = Database().with { $0.name = ... }
  /// ```
  public func with(_ config: (inout Self) throws -> Swift.Void) rethrows -> Self {
    var copy = self
    try config(&copy)
    return copy
  }

  private struct CodingKeys: CodingKey {
    var stringValue: Swift.String
    var intValue: Swift.Int? { nil }
    init(stringValue: Swift.String) { self.stringValue = stringValue }
    init?(intValue: Swift.Int) { nil }

    static let name = CodingKeys(stringValue: "name")
    static let charset = CodingKeys(stringValue: "charset")
    static let collation = CodingKeys(stringValue: "collation")
    static let characterType = CodingKeys(stringValue: "characterType")
    static let databaseTemplate = CodingKeys(stringValue: "databaseTemplate")
    static let isTemplateDatabase = CodingKeys(stringValue: "isTemplateDatabase")

    static let _knownKeys: Set<Swift.String> = [
      "name",
      "charset",
      "collation",
      "characterType",
      "databaseTemplate",
      "isTemplateDatabase",
    ]
  }

  public init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: CodingKeys.self)
    if let value = try container.decodeIfPresent(Swift.String.self, forKey: .name) {
      self.name = value
    }
    if let value = try container.decodeIfPresent(Swift.String.self, forKey: .charset) {
      self.charset = value
    }
    if let value = try container.decodeIfPresent(Swift.String.self, forKey: .collation) {
      self.collation = value
    }
    if let value = try container.decodeIfPresent(Swift.String.self, forKey: .characterType) {
      self.characterType = value
    }
    if let value = try container.decodeIfPresent(Swift.String.self, forKey: .databaseTemplate) {
      self.databaseTemplate = value
    }
    self.isTemplateDatabase = try container.decodeIfPresent(
      Swift.Bool.self, forKey: .isTemplateDatabase)
    for key in container.allKeys where !CodingKeys._knownKeys.contains(key.stringValue) {
      self._unknownFields.json[key.stringValue] = try container.decode(
        GoogleWKT.WKTValue.self, forKey: key)
    }
  }

  public func encode(to encoder: Encoder) throws {
    var container = encoder.container(keyedBy: CodingKeys.self)
    try container.encode(self.name, forKey: .name)
    try container.encode(self.charset, forKey: .charset)
    try container.encode(self.collation, forKey: .collation)
    try container.encode(self.characterType, forKey: .characterType)
    try container.encode(self.databaseTemplate, forKey: .databaseTemplate)
    try container.encodeIfPresent(self.isTemplateDatabase, forKey: .isTemplateDatabase)
    for (key, value) in self._unknownFields.json {
      try container.encode(value, forKey: CodingKeys(stringValue: key))
    }
  }

  public static var _anyTypeUrl: Swift.String {
    return "type.googleapis.com/google.cloud.alloydb.v1.Database"
  }
  public init(fromAny any: GoogleWKT.WKTAny) throws {
    self = try GoogleWKT._slowAnyDeserialize(Self.self, from: any)
  }
  public func _pack() throws -> GoogleWKT.WKTStruct {
    return try GoogleWKT._slowAnySerialize(message: self)
  }
}
