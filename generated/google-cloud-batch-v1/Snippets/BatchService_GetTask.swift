...
func sample(
  ...
  taskGroupId: String, taskId: String
) async throws {
  let response = try await client.getTask(
    request: GetTaskRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/jobs/\(jobId)/taskGroups/\(taskGroupId)/tasks/\(taskId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        jobId: "[placeholder]", taskGroupId: "[placeholder]", taskId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
