...
func sample(
  ...
  let response = try await client.updateAutoscalingPolicy(
    request: UpdateAutoscalingPolicyRequest()
      .with {
        $0.policy = AutoscalingPolicy().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/autoscalingPolicies/\(autoscalingPolicyId)"
        }
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        autoscalingPolicyId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
