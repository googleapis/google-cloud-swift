...
Add `swift-google-cloud-aiplatform-v1-schema-predict-prediction` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-aiplatform-v1-schema-predict-prediction.git --from 0.3.0
```

Then add `GoogleCloudAIPlatformV1SchemaPredictPrediction` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudAIPlatformV1SchemaPredictPrediction <target-name> --package swift-google-cloud-aiplatform-v1-schema-predict-prediction
```
