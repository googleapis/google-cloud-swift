...
extension Clients {
  final class ServiceControllerTransport: ServiceControllerStub {
    ...
    public func check(
      ...
      let (path, query, configure, omitted) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v2/services/\(pathVariable0):check"
          let query = [
      ...
      return try await req.rpc(
        GoogleApiServiceControlV2.CheckResponse.self, timeout: options.attemptTimeout
      ).get()
    }
    ...
    public func report(
      ...
      let (path, query, configure, omitted) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v2/services/\(pathVariable0):report"
          let query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          return (path, query)
      ...
      try req.setBody(json: request, omitting: omitted)
      return try await req.rpc(
        GoogleApiServiceControlV2.ReportResponse.self, timeout: options.attemptTimeout
      ).get()
    }
  }
}
