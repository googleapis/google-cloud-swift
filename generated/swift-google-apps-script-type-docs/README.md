...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-apps-script-type-docs.git --from 0.0.0-preview
```

Then add `GoogleAppsScriptTypeDocs` to your target's dependencies:

```bash
swift package add-target-dependency GoogleAppsScriptTypeDocs <target-name> --package swift-google-apps-script-type-docs
```
