...
extension Clients {
  final class OracleDatabaseLogging: OracleDatabaseStub {
    let inner: any OracleDatabaseStub
    let logger: Logger

    public init(_ inner: any OracleDatabaseStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-oracledatabase-v1"
      logger[metadataKey: "gcp.client.service"] = "oracledatabase"
      logger[metadataKey: "gcp.experimental.swift.client"] = "OracleDatabase"
      self.inner = inner
      self.logger = logger
    }
