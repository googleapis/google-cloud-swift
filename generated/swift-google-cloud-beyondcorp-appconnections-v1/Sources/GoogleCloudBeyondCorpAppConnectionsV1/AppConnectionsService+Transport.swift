...
extension Clients {
  final class AppConnectionsServiceTransport: AppConnectionsServiceStub {
    ...
    public func listAppConnections(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/appConnections"
          var query = [
    ...
    public func updateAppConnection(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.appConnection.map({ $0.name }),
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/appConnections/"), .singleWildcard,
              ],
              fieldName: "app_connection.name")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0)"
    ...
    public func resolveAppConnections(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/appConnections:resolve"
          var query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          let encoder = GoogleGax._QueryParameterEncoder()
