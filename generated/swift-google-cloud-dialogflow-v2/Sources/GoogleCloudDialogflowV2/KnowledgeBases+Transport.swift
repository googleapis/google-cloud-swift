...
// limitations under the License.
  ...
  extension Clients {
    final class KnowledgeBasesTransport: KnowledgeBasesStub {
      ...
      public func listKnowledgeBases(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/v2/\(pathVariable0)/knowledgeBases"
            var query = [
      ...
      public func updateKnowledgeBase(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            guard
              let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
                request.knowledgeBase.map({ $0.name }),
                matching: [
                  .literal("projects/"), .singleWildcard, .literal("/knowledgeBases/"),
                  .singleWildcard,
                ],
                fieldName: "knowledge_base.name")
            else {
              return nil
            }
            let path = "/v2/\(pathVariable0)"
