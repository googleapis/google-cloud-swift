...
extension Clients {
  final class SequenceServiceTransport: SequenceServiceStub {
    ...
    public func attemptSequence(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = GoogleCloudGax._RoutingMatcher.value(
              request.name as Swift.String?, matching: [.literal("sequences/"), .singleWildcard])
          else {
            return nil
          }
          let path = "/v1beta1/\(pathVariable0)"
    ...
    public func attemptStreamingSequence(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = GoogleCloudGax._RoutingMatcher.value(
              request.name as Swift.String?,
              matching: [.literal("streamingSequences/"), .singleWildcard])
          else {
            return nil
          }
          let path = "/v1beta1/\(pathVariable0):stream"
          let query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          return (path, query)
      ...
      try req.setBody(json: request)
      return try await req.rpc(
        GoogleShowcaseV1Beta1.AttemptStreamingSequenceResponse.self, timeout: options.attemptTimeout
      ).get()
    }
