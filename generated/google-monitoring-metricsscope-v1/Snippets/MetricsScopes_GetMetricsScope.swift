...
func sample(client: MetricsScopesClient, metricsScopeId: String) async throws {
  let response = try await client.getMetricsScope(
    request: GetMetricsScopeRequest()
      .with {
        $0.name = "locations/global/metricsScope/\(metricsScopeId)"
      }
  )
...
struct SnippetRunner {
  static func main() async throws {
    do {
      ...
      try await sample(client: client, metricsScopeId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
