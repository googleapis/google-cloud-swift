...
// limitations under the License.
  ...
  extension Clients {
    final class TargetGrpcProxiesTransport: TargetGrpcProxiesStub {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable1 = try GoogleGax._RoutingMatcher.pathValue(
                request.targetGrpcProxy as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "targetGrpcProxy")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/global/targetGrpcProxies/\(pathVariable1)"
            var query = [
      ...
      public func `get`(
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.TargetGrpcProxy.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func insert(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/global/targetGrpcProxies"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            let encoder = GoogleGax._QueryParameterEncoder()
      ...
      public func list(
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.TargetGrpcProxyList.self, timeout: options.attemptTimeout
        ).get()
      }
