...
// limitations under the License.
  ...
  extension Clients {
    final class NetworkProfilesTransport: NetworkProfilesStub {
      ...
      public func `get`(
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable1 = try GoogleGax._RoutingMatcher.pathValue(
                request.networkProfile as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "networkProfile")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/global/networkProfiles/\(pathVariable1)"
            let query = [
        ...
        return try await req.rpc(
          GoogleCloudComputeV1.NetworkProfile.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func list(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/global/networkProfiles"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.NetworkProfilesListResponse.self, timeout: options.attemptTimeout
        ).get()
      }
    }
  }
