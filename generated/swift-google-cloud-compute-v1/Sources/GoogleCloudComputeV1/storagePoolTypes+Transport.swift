...
// limitations under the License.
  ...
  extension Clients {
    final class StoragePoolTypesTransport: StoragePoolTypesStub {
      ...
      public func aggregatedList(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/aggregated/storagePoolTypes"
            var query = [
        ...
        return try await req.rpc(
          GoogleCloudComputeV1.StoragePoolTypeAggregatedList.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func `get`(
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable2 = try GoogleGax._RoutingMatcher.pathValue(
                request.storagePoolType as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "storagePoolType")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/zones/\(pathVariable1)/storagePoolTypes/\(pathVariable2)"
            let query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            return (path, query)
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.StoragePoolType.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func list(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path =
              "/compute/v1/projects/\(pathVariable0)/zones/\(pathVariable1)/storagePoolTypes"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            let encoder = GoogleGax._QueryParameterEncoder()
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.StoragePoolTypeList.self, timeout: options.attemptTimeout
        ).get()
      }
    }
  }
