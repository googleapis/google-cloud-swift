    ...
    final class InterconnectLocationsTransport: InterconnectLocationsStub {
      ...
      public func `get`(
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable1 = try GoogleGax._RoutingMatcher.pathValue(
                request.interconnectLocation as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "interconnectLocation")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/global/interconnectLocations/\(pathVariable1)"
            let query = [
        ...
        return try await req.rpc(
          GoogleCloudComputeV1.InterconnectLocation.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func list(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/global/interconnectLocations"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.InterconnectLocationList.self, timeout: options.attemptTimeout
        ).get()
      }
    }
  }
