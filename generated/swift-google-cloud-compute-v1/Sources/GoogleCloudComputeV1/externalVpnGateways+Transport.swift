          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable1 = try GoogleGax._RoutingMatcher.pathValue(
                request.externalVpnGateway as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "externalVpnGateway")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/global/externalVpnGateways/\(pathVariable1)"
            var query = [
      ...
      public func insert(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/global/externalVpnGateways"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            let encoder = GoogleGax._QueryParameterEncoder()
