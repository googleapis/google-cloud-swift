      ...
      public func aggregatedList(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/aggregated/futureReservations"
            var query = [
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable2 = try GoogleGax._RoutingMatcher.pathValue(
                request.futureReservation as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "futureReservation")
            else {
              return nil
            }
            let path =
      ...
      public func insert(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path =
              "/compute/v1/projects/\(pathVariable0)/zones/\(pathVariable1)/futureReservations"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            let encoder = GoogleGax._QueryParameterEncoder()
