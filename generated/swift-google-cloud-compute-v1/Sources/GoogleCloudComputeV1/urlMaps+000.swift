...
// limitations under the License.
  ...
  public final class UrlMapsClient: Clients.UrlMapsProtocol, Sendable {
    ...
    public func invalidateCachePollingUntilDone(
      ...
      let rawOp = try await self.invalidateCache(request: request, options: options)
      let initialState = try extractStatus(rawOp)
      let poll = {
        () async throws -> GoogleGax._PollableOperationImpl<GoogleCloudComputeV1.Operation>.State in
        let op = try await self.getOperation(
  ...
  extension Clients.UrlMapsProtocol {
    ...
    public func invalidateCachePollingUntilDone(
      ...
      return try await self.invalidateCachePollingUntilDone(request: request)
    }
  ...
  @available(
    *, unavailable, message: "Enable the 'UrlMaps' trait in Package.swift to use this client."
  )
  public final class UrlMapsClient: Sendable {
    @available(
      *, unavailable, message: "Enable the 'UrlMaps' trait in Package.swift to use this client."
    )
    public init(_ options: GoogleGax.ClientOptions = .init()) throws {}
