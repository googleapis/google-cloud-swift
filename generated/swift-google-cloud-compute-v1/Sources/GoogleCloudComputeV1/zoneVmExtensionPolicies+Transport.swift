  ...
  extension Clients {
    final class ZoneVmExtensionPoliciesTransport: ZoneVmExtensionPoliciesStub {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable2 = try GoogleGax._RoutingMatcher.pathValue(
                request.vmExtensionPolicy as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "vmExtensionPolicy")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/zones/\(pathVariable1)/vmExtensionPolicies/\(pathVariable2)"
            var query = [
      ...
      public func `get`(
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.VmExtensionPolicy.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func insert(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path =
              "/compute/v1/projects/\(pathVariable0)/zones/\(pathVariable1)/vmExtensionPolicies"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            let encoder = GoogleGax._QueryParameterEncoder()
      ...
      public func list(
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.VmExtensionPolicyList.self, timeout: options.attemptTimeout
        ).get()
      }
