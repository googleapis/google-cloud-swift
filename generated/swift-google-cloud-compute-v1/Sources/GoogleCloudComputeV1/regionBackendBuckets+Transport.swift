...
// limitations under the License.
  ...
  extension Clients {
    final class RegionBackendBucketsTransport: RegionBackendBucketsStub {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable2 = try GoogleGax._RoutingMatcher.pathValue(
                request.backendBucket as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "backendBucket")
            else {
              return nil
            }
            let path =
      ...
      public func insert(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path =
              "/compute/v1/projects/\(pathVariable0)/regions/\(pathVariable1)/backendBuckets"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            let encoder = GoogleGax._QueryParameterEncoder()
