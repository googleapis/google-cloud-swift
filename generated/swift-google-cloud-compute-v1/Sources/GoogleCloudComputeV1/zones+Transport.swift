...
// limitations under the License.
  ...
  extension Clients {
    final class ZonesTransport: ZonesStub {
      ...
      public func `get`(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/zones/\(pathVariable1)"
            let query = [
        ...
        return try await req.rpc(
          GoogleCloudComputeV1.Zone.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func list(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/zones"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.ZoneList.self, timeout: options.attemptTimeout
        ).get()
      }
    }
  }
