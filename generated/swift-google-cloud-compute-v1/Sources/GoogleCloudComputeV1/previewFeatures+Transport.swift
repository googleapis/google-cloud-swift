...
// limitations under the License.
  ...
  extension Clients {
    final class PreviewFeaturesTransport: PreviewFeaturesStub {
      ...
      public func `get`(
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable1 = try GoogleGax._RoutingMatcher.pathValue(
                request.previewFeature as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "previewFeature")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/global/previewFeatures/\(pathVariable1)"
            let query = [
        ...
        return try await req.rpc(
          GoogleCloudComputeV1.PreviewFeature.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func list(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/global/previewFeatures"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.PreviewFeatureList.self, timeout: options.attemptTimeout
        ).get()
      }
