...
// limitations under the License.
  ...
  extension Clients {
    final class RegionAutoscalersTransport: RegionAutoscalersStub {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            guard
              let pathVariable2 = try GoogleGax._RoutingMatcher.pathValue(
                request.autoscaler as Swift.String?,
                matching: [.singleWildcard],
                fieldName: "autoscaler")
            else {
              return nil
            }
            let path =
              "/compute/v1/projects/\(pathVariable0)/regions/\(pathVariable1)/autoscalers/\(pathVariable2)"
            var query = [
      ...
      public func insert(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path = "/compute/v1/projects/\(pathVariable0)/regions/\(pathVariable1)/autoscalers"
            var query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            let encoder = GoogleGax._QueryParameterEncoder()
      ...
      public func list(
        ...
        req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
        return try await req.rpc(
          GoogleCloudComputeV1.RegionAutoscalerList.self, timeout: options.attemptTimeout
        ).get()
      }
      ...
      public func patch(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let encoder = GoogleGax._QueryParameterEncoder()
            query.append(contentsOf: try encoder.encode(request.autoscaler, prefix: "autoscaler"))
            query.append(contentsOf: try encoder.encode(request.requestId, prefix: "requestId"))
            return (path, query)
          }() {
      ...
      public func testIamPermissions(
        ...
        let (path, query, configure) = try {
          ...
          if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
            ...
            let path =
              "/compute/v1/projects/\(pathVariable0)/regions/\(pathVariable1)/autoscalers/\(pathVariable2)/testIamPermissions"
            let query = [
              URLQueryItem(name: "$alt", value: "json")
            ]
            return (path, query)
