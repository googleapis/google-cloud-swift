...
{
  let response = try await client.getDataSource(
    request: GetDataSourceRequest()
      .with {
        $0.name = "projects/\(projectId)/dataSources/\(dataSourceId)"
      }
  )
...
struct SnippetRunner {
  static func main() async throws {
    do {
      ...
      try await sample(client: client, projectId: "[placeholder]", dataSourceId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
