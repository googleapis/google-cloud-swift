...
extension Clients {
  final class ProvisioningTransport: ProvisioningStub {
    ...
    public func createApiHubInstance(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/apiHubInstances"
          var query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          let encoder = GoogleGax._QueryParameterEncoder()
          query.append(
            contentsOf: try encoder.encode(request.apiHubInstanceId, prefix: "apiHubInstanceId"))
          return (path, query)
        }() {
      ...
      if let body = request.apiHubInstance {
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              ...
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/apiHubInstances/"), .singleWildcard,
              ],
              fieldName: "name")
          else {
            return nil
        ...
        do {
          ...
          builder.maybeAdd(
            ...
            fieldName: "name",
            expecting: "projects/*/locations/*/apiHubInstances/*"
          )
          paths.append(builder.build())
        }
        throw GoogleGax.RequestError.binding(GoogleGax.BindingError(paths: paths))
    ...
    public func getApiHubInstance(
      ...
      req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
      return try await req.rpc(
        GoogleCloudApiHubV1.ApiHubInstance.self, timeout: options.attemptTimeout
      ).get()
    }
    ...
    public func lookupApiHubInstance(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/apiHubInstances:lookup"
          let query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          return (path, query)
      ...
      req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
      return try await req.rpc(
        GoogleCloudApiHubV1.LookupApiHubInstanceResponse.self, timeout: options.attemptTimeout
      ).get()
    }
