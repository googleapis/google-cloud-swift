...
extension Clients {
  final class ApiHubCurateTransport: ApiHubCurateStub {
    ...
    public func createCuration(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/curations"
          var query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          let encoder = GoogleGax._QueryParameterEncoder()
          query.append(contentsOf: try encoder.encode(request.curationId, prefix: "curationId"))
          return (path, query)
        }() {
      ...
      if let body = request.curation {
        try req.setBody(json: body)
      }
      return try await req.rpc(
        GoogleCloudApiHubV1.Curation.self, timeout: options.attemptTimeout
      ).get()
    }

    public func getCuration(
      request: GetCurationRequest, options: GoogleGax.RequestOptions
    ) async throws -> GoogleCloudApiHubV1.Curation {
      let (path, query, configure) = try {
        () throws -> (Swift.String, [URLQueryItem], (inout GoogleGax._HTTPClientRequest) -> Void) in
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              ...
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/curations/"), .singleWildcard,
              ],
              fieldName: "name")
          else {
            return nil
        ...
        do {
          ...
          builder.maybeAdd(
            ...
            fieldName: "name",
            expecting: "projects/*/locations/*/curations/*"
          )
          paths.append(builder.build())
        }
        throw GoogleGax.RequestError.binding(GoogleGax.BindingError(paths: paths))
    ...
    public func listCurations(
      ...
      req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
      return try await req.rpc(
        GoogleCloudApiHubV1.ListCurationsResponse.self, timeout: options.attemptTimeout
      ).get()
    }

    public func updateCuration(
      request: UpdateCurationRequest, options: GoogleGax.RequestOptions
    ) async throws -> GoogleCloudApiHubV1.Curation {
      let (path, query, configure) = try {
        () throws -> (Swift.String, [URLQueryItem], (inout GoogleGax._HTTPClientRequest) -> Void) in
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.curation.map({ $0.name }),
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/curations/"), .singleWildcard,
              ],
              fieldName: "curation.name")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0)"
    ...
    public func deleteCuration(
      request: DeleteCurationRequest, options: GoogleGax.RequestOptions
    ) async throws {
      let (path, query, configure) = try {
        () throws -> (Swift.String, [URLQueryItem], (inout GoogleGax._HTTPClientRequest) -> Void) in
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
