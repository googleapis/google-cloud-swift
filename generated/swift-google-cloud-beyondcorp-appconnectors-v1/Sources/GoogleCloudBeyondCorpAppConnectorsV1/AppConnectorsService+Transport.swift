...
extension Clients {
  final class AppConnectorsServiceTransport: AppConnectorsServiceStub {
    ...
    public func listAppConnectors(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/appConnectors"
          var query = [
    ...
    public func createAppConnector(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let encoder = GoogleGax._QueryParameterEncoder()
          query.append(
            contentsOf: try encoder.encode(request.appConnectorId, prefix: "appConnectorId"))
    ...
    public func updateAppConnector(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.appConnector.map({ $0.name }),
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/appConnectors/"), .singleWildcard,
              ],
              fieldName: "app_connector.name")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0)"
    ...
    public func reportStatus(
      ...
      let (path, query, configure, omitted) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0):reportStatus"
          let query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          return (path, query)
        }() {
          return (candidate.0, candidate.1, { $0.setMethod(.POST) }, ["appConnector"])
        }
        var paths: [GoogleGax.PathMismatch] = []
        do {
          var builder = GoogleGax._PathMismatchBuilder()
