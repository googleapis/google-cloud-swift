...
func sample(
  ...
  let response = try await client.getMigrationSubtask(
    request: GetMigrationSubtaskRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/workflows/\(workflowId)/subtasks/\(subtaskId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        workflowId: "[placeholder]", subtaskId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
