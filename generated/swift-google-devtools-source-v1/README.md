...
Add `swift-google-devtools-source-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-devtools-source-v1.git --from 0.0.0-preview
```

Then add `GoogleDevToolsSourceV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleDevToolsSourceV1 <target-name> --package swift-google-devtools-source-v1
```
