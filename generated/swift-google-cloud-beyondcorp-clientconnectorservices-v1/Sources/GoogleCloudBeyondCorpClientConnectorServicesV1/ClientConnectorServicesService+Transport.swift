...
extension Clients {
  final class ClientConnectorServicesServiceTransport: ClientConnectorServicesServiceStub {
      ...
      -> GoogleCloudBeyondCorpClientConnectorServicesV1.ListClientConnectorServicesResponse
    {
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/clientConnectorServices"
          var query = [
    ...
    public func updateClientConnectorService(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.clientConnectorService.map({ $0.name }),
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/clientConnectorServices/"), .singleWildcard,
              ],
              fieldName: "client_connector_service.name")
          else {
            return nil
          }
          let path = "/v1/\(pathVariable0)"
