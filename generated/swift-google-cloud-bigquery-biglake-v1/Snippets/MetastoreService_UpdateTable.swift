  ...
  let response = try await client.updateTable(
    request: UpdateTableRequest()
      .with {
        $0.table = Table().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/catalogs/\(catalogId)/databases/\(databaseId)/tables/\(tableId)"
        }
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        catalogId: "[placeholder]", databaseId: "[placeholder]", tableId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
