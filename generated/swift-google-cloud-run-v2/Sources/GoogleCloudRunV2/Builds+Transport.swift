...
extension Clients {
  final class BuildsTransport: BuildsStub {
    ...
    public func submitBuild(
      request: SubmitBuildRequest, options: GoogleGax.RequestOptions
      ...
      let (path, query, configure, omitted) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v2/\(pathVariable0)/builds:submit"
          let query = [
      ...
      return try await req.rpc(
        GoogleCloudRunV2.SubmitBuildResponse.self, timeout: options.attemptTimeout
      ).get()
    }
