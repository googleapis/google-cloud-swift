...
/// tracks forward transfer progress and handles transient error recovery, session resumption,
/// and backoff across an entire multi-step transfer session using a `ResumePolicy` and `BackoffPolicy`.
public struct _ResumeLoop: Sendable {
  ...
  /// Updates transfer forward progress, advancing bytes transferred and resetting consecutive errors.
  ///
  /// - Parameters:
  ///   - state: The transfer `ResumeState` to update.
  ///   - bytesAdvanced: The number of new bytes transferred or committed.
  public func onProgress(state: inout ResumeState, bytesAdvanced: UInt64) {
    resumePolicy.onProgress(state: &state, bytesAdvanced: bytesAdvanced)
  }

  /// Handles an error encountered during a step of a resumable transfer operation.
  ///
  ...
  ///
  /// - Parameters:
  ///   - state: The mutable transfer `ResumeState` to update.
  ...
  ///
  /// - Parameters:
  ///   - state: The mutable transfer `ResumeState` updated across attempts.
