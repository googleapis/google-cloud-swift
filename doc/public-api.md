...
- **Package Products**: Swift Package Manager library products explicitly declared under `products:` in each `Package.swift` (e.g., `GoogleCloudStorage`, `GoogleAuth`, `GoogleGax`, `GoogleWKT`).
...
### Enum Evolution and Switching

- **Non-Frozen Enums**: Public enumerations in `google-cloud-swift` packages (both handwritten packages and generated client libraries) are non-frozen. As Google Cloud APIs and SDK capabilities evolve, new cases may be added to existing enumerations in minor or patch releases. Adding a new enum case is **not** considered a breaking change.
- **Handling Unknown Cases**: When writing exhaustive `switch` statements over SDK enums, consumers must always include an `@unknown default:` (or `default:`) clause to ensure forward source compatibility. Omitting a catch-all may cause compilation errors when updating to newer SDK versions.
- **Catch-All Payload Promotion (`StatusDetail.other` and `unknownStringValue`)**:
  - In `GoogleGax.StatusDetail`, unknown or unparsed status detail messages fall into `.other(GoogleWKT.WKTAny)`. Do not match `.other` expecting specific protobuf types to remain there indefinitely; as the SDK adds support for more standard or service-specific detail types, those payloads will be parsed into their own dedicated enum cases.
  - In generated enums, wire values unrecognized by the current SDK version are captured in `unknownStringValue` or `unknownIntValue`. Do not pattern-match specific literals (such as `.unknownStringValue("NEW_FEATURE")`), as a future minor release will promote known values to named cases (e.g., `.newFeature`), causing existing `.unknownStringValue` patterns to silently stop matching.
