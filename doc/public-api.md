...
- **Catch-All Payload Promotion (`StatusDetail.other` and `unknownStringValue`)**:
  - In `GoogleGax.StatusDetail`, unknown or unparsed status detail messages fall into `.other(GoogleWKT.Any)`. Do not match `.other` expecting specific protobuf types to remain there indefinitely; as the SDK adds support for more standard or service-specific detail types, those payloads will be parsed into their own dedicated enum cases.
