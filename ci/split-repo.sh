...
# path from the repository root (e.g., pkgs/swift-google-auth, generated/swift-google-rpc)
...
${COLOR_BOLD}DESCRIPTION${COLOR_RESET}
    Splits a subdirectory path (e.g., 'pkgs/swift-google-auth', 'generated/swift-google-rpc')
...
${COLOR_BOLD}ARGUMENTS${COLOR_RESET}
    <PATH>                      Relative path from repository root (e.g. 'pkgs/swift-google-auth',
...
${COLOR_BOLD}EXAMPLES${COLOR_RESET}
    # 1. Split 'pkgs/swift-google-auth' and output the commit SHA:
    ${0##*/} pkgs/swift-google-auth
    ...
    ${0##*/} -p pkgs/swift-google-auth -b split/swift-google-auth
    ...
    ${0##*/} pkgs/swift-google-auth -o v1.0.0 --tag v1.0.0 -r git@github.com:googleapis/swift-google-auth.git --push-tag
EOF
}
...
main() {
    ...
    if [[ -z "${target_path}" ]]; then
        log_error "Missing required target path. Example: '${0##*/} pkgs/swift-google-auth' or '${0##*/} generated/swift-google-rpc'."
        echo "Run '${0##*/} --help' for usage."
        exit 1
    fi
