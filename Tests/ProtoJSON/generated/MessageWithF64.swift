...
public struct MessageWithF64: Codable, Equatable, GoogleWKT._AnyPackable,
  Sendable
{
  /// A singular field.
  ...
  public init(from decoder: Decoder) throws {
    ...
    if let value = try container.decodeIfPresent(Swift.Double.self, forKey: .singular) {
      self.singular = value
    }
    self.option = try container.decodeIfPresent(Swift.Double.self, forKey: .option)
    if let value = try container.decodeIfPresent([Swift.Double].self, forKey: .repeated) {
      self.repeated = value
    }
    if let value = try container.decodeIfPresent([Swift.String: Swift.Double].self, forKey: .map) {
      self.map = value
    }
